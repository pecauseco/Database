...
void db_query(char *name, char *result, int len) {
    ...
    int err;
    node_t *target;
    if ((err = pthread_rwlock_rdlock(&(&head)->rwlock)) != 0) {
    ...
    if (target == 0) {
        ...
        if ((err = pthread_rwlock_unlock(&(target)->rwlock)) != 0) {
            fprintf(stderr, "lock error");
            exit(1);
        }
        return;
...
int db_add(char *name, char *value) {
    ...
    node_t *target;
    node_t *newnode;
    if ((err = pthread_rwlock_wrlock(&(&head)->rwlock)) != 0) {
        fprintf(stderr, "lock error");
        exit(1);
    }
    if ((target = search(name, &head, &parent, 1)) != 0) {
        ...
        if ((err = pthread_rwlock_unlock(&parent->rwlock) != 0)) {
            fprintf(stderr, "lock error");
            exit(1);
        }
        return (0);
    ...
    if ((err = pthread_rwlock_init(&newnode->rwlock, 0)) != 0) {
...
int db_remove(char *name) {
    ...
    if (dnode->rchild == 0) {
        ...
        if ((pthread_rwlock_unlock(&(dnode->rwlock))) != 0) {
            fprintf(stderr, "lock error");
            exit(1);
        }
        if ((pthread_rwlock_unlock(&(parent->rwlock))) != 0) {
            fprintf(stderr, "lock error");
            exit(1);
        }
        node_destructor(dnode);  // maybe this goes before locks
        ...
        // done with dnode
        if ((err = pthread_rwlock_unlock(&(dnode)->rwlock)) != 0) {
            fprintf(stderr, "lock error");
            exit(1);
        }
        if ((err = pthread_rwlock_unlock(&(parent)->rwlock)) != 0) {
        ...
        if ((err = pthread_rwlock_wrlock(&(next)->rwlock)) != 0) {
            fprintf(stderr, "lock error");
            exit(1);
        }
        ...
        while (next->lchild != 0) {
            ...
            if ((err = pthread_rwlock_wrlock(&(nextl)->rwlock)) != 0) {
                fprintf(stderr, "lock error");
                exit(1);
            }
            pnext = &next->lchild;
            if ((err = pthread_rwlock_unlock(&(next)->rwlock)) != 0) {
                fprintf(stderr, "lock error");
                exit(1);
            }  // unlock current next
            next = nextl;
...
node_t *search(char *name, node_t *parent, node_t **parentpp, int rorw) {
    ...
    if (next == NULL) {
        result = NULL;  // do i need to lock here?
    } else {
        if (rorw == 0) {
            if ((err = pthread_rwlock_rdlock(&next->rwlock)) != 0) {
                fprintf(stderr, "lock error");
                exit(1);
            }
        } else if (rorw == 1) {
        ...
        if (strcmp(name, next->name) == 0) {
            ...
            return search(name, next, parentpp, rorw);
        }
    }
...
void interpret_command(char *command, char *response, int len) {
    ...
    // pass in mutex locked to cond wait
    // unlock mutex after cond wait
}
